...
if [[ -n $pane_file ]] && valid_pane_file "$pane_file"; then
	...
	# A nested agent — `claude -p` from a hook, script or tool call — inherits
	# $TMUX_PANE and so resolves this same manifest key, then reports its own
	# fresh `startup`. Clearing there wipes the carousel of the session that
	# spawned it (#234). Skip every branch while the recorded owner is a
	# different, still-live session; a dead or unrecorded owner is the exited
	# agent these clears are actually for.
