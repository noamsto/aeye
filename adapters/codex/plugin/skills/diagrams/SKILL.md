...
Rendering needs `aeye` (it embeds the d2 compiler; the hook runs
`aeye render-diagram`) and `resvg` on PATH. If either is missing the hook no-ops
silently (no diagram, no error) — install both to enable diagrams. Theme choice
and the contrast pass run inside `aeye`: the label-recolor backstop re-inks the
label of any hand-filled node, whether the fill is a `#hex` or a CSS name like
`green`, which is why the stroke-coded diagrams this skill recommends stay
