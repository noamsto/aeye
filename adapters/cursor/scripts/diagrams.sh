...
while IFS= read -r p; do
	...
	[[ ${p,,} == *.d2 ]] || continue
	candidates+=("$p")
done < <(cursor_extract_touched_paths "$payload")
...
for candidate in "${candidates[@]}"; do
	# Canonical variant recorded in the manifest; the carousel swaps -dark/-light
	# to the live theme at view time. d2_render below renders both variants.
	...
	# (exact: no source-grep false positives, catches every markdown syntax).
	# Suppress the blank render entirely — drop it from disk, never add it to
	# the manifest or open the carousel — and warn the agent so the corrected
	# re-render is the only one shown. Deleting it also re-arms this check,
	# since the fixed re-render renders fresh.
	...
	# below against a concurrent images.sh append of the same manifest. Taken
	# after the (slow) d2_render above so rendering never holds the lock.
	_manifest_lock "$LOCK_FILE"

	# Self-heal against tmux pane-id reuse: a manifest last written by a
	# different Cursor session belongs to a pane that's since been recycled —
	# drop it so this session's carousel never blends in a prior session's
	# images. (The SessionStart reset already covers fresh starts; this also
	# guards a start the reset missed.)
	owner_selfheal "$pane_file" "$session"

	# Append guarded by a path-dedup check (independent of the render step, so
	# a diagram missing from the manifest is re-added even when its PNG is
	# cached).
	if [[ -f $manifest ]] &&
	...
	# Prune renders this edit supersedes: an edited .d2 hashes to a new png, so
	# the prior hash would otherwise linger forever in the manifest and on
	# disk. Drop same-name/different-path entries from this manifest, then GC
	# their files — unless another pane's manifest still references the
	# render (panes share the content-hashed dir, so a file in use elsewhere
	# must survive).
	if [[ -f $manifest ]]; then
	...
	# Proactively surface the carousel on every new diagram. Reached only when
	# a genuinely new diagram was appended above (the dedup guard `continue`s
	# for ones already in the manifest), so this re-opens after a manual close
	# but never re-fires for an unchanged redraw. --ensure-open is idempotent
	# and never kills.
