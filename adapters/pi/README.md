...
D2 diagram renders into the aeye carousel. Ships as a [pi package][pi-pkg]:
[hookyard](https://github.com/earendil-works/hookyard) "build mode" handlers
plus the `image-gallery` / `diagrams` skills.

[pi-pkg]: https://github.com/earendil-works/pi/blob/main/packages/coding-agent/docs/packages.md

Hookyard translates pi's lifecycle events into the same normalized hook
payload the Claude/Codex/Cursor adapters feed their scripts, dispatching to
the handler wrapper scripts under `handlers/`, which run the shell capture
scripts under `scripts/` and inject the SessionStart diagram guidance and any
diagram compile warning back into the conversation.
...
loads the built plugin from `extensions/` (generated by `hookyard build
--engine pi` from `hookyard.json` and the `handlers/`/`commands/` scripts, and
committed to git so install works right after clone) and the skills from
`skills/`. To try it without installing, load the built extension file
directly:
...
Maintainers changing `hookyard.json` or the handler/command scripts must
regenerate the build artifacts with `just build-pi-plugin` (requires
`hookyard` on PATH and `HOOKYARD_SRC` set — see the justfile recipe) and
commit the result.
...
- **tmux-primary** — designed for pi in tmux, where the pane key is shared with
  ...
  `/aeye` command exports `$AEYE_SESSION_ID` so the toggle the agent runs finds
  it, but a carousel opened from an unrelated shell needs that variable set.
